...
public class HighlightsFX : MonoBehaviour 
{
    ...
    public void changeObject(GameObject go)
    {
        ...
        if ( g == null)
        {
            Destroy(gameObject.GetComponent<BlurOptimized>());
        }
        else
        {
            GetAllRenderer(g);
	...
	private void RenderHighlights( RenderTexture rt)
	{
        ...
        {
            m_renderBuffer.DrawRenderer(renderers[i], m_highlightMaterial, 0, (int)m_sortingType);
        }
    ...
    private void GetAllRenderer(GameObject o)
    {
        Renderer r = o.GetComponent<Renderer>();
        if (r != null)
        {
            int length = renderers.Length;
            System.Array.Resize<Renderer>(ref renderers, length + 1);
            renderers[length] = r;
        }
        else
        {
